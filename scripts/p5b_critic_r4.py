"""
Phase 5 critic (DETAIL+ANIM pass, round 4) — beauty + deform-test renders.
...
GPU (OptiX/CUDA) device is enabled (CPU fallback = hard failure), then renders:
  ...
  1. renders/wip/p5b_r4_full.png   — full-body beauty (Cam_Full fallback chain)
  2. renders/wip/p5b_r4_face.png   — face close-up (Cam_Face)
  3. renders/wip/p5b_r4_deform.png — DEFORM TEST: bent arm + elbow crook +
...
Run:
  blender --background models/godwyn_phase1.blend \
      --python scripts/p5b_critic_r4.py
"""
import math
import os
import sys
...
def anim_audit():
    ...
    print(f"[p5b r4] ARMATURES ({len(arms)}): {[a.name for a in arms]}")
    ...
    for m in meshes:
        ...
        print(f"[p5b r4]  mesh {m.name}: skinned={skinned} "
    ...
    print(f"[p5b r4] SHAPE KEYS ({len(keys)}): {keys}")
    ...
    for mat in bpy.data.materials:
        if mat.use_nodes and mat.node_tree:
            ...
            if dm == "DISPLACEMENT" or dm == "BOTH":
                print(f"[p5b r4]  NOTE: material {mat.name} uses geometry "
...
def main():
    ...
    print(f"[p5b r4] GPU OK: {dev_type} — " + ", ".join(d.name for d in gpus))
    ...
    def shoot(cam, out_name, rx, ry):
        ...
        print(f"[p5b r4] rendering {cam} -> {out}")
        bpy.ops.render.render(write_still=True)
        assert os.path.isfile(out) and os.path.getsize(out) > 1024, (
            f"FATAL: render missing: {out}")
        print(f"[p5b r4] wrote {out} ({os.path.getsize(out)//1024} KB)")

    full_cam = _pick(_FULL_CANDIDATES)
    shoot(full_cam, "p5b_r4_full.png", 1280, 1600)
    shoot("Cam_Face", "p5b_r4_face.png", 1280, 1280)
    ...
    print(f"[p5b r4] grip bones curled: {len(grip)} -> {grip[:6]}")
    ...
    if kb is None:
        ...
        kb = others[0]
    kb.value = 0.7
    print(f"[p5b r4] deform pose applied; blendshape '{kb.name}'=0.7")
    ...
    for i, v in enumerate(kb.data):
        ...
        if d > 1e-6:
            moved += 1
            mx = max(mx, d)
    print(f"[p5b r4] blendshape '{kb.name}': {moved} verts moved, "
          f"max delta {mx*1000:.2f} mm (x0.7 driven at render)")
    assert moved > 0, "FATAL: blendshape has zero vertex deltas"

    shoot(_pick(_DEFORM_CANDIDATES), "p5b_r4_deform.png", 1280, 1600)
    # face close-up of the DRIVEN expression so the blendshape is judgeable
    shoot("Cam_Face", "p5b_r4_deform_face.png", 1280, 1280)
    print("[p5b r4] critic renders complete (scene NOT saved).")
