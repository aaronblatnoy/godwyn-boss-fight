"""
Movement-eval round 2 — animation frames from godwyn_p2_robe.blend.
...
4. Renders 4 frames across the motion, 3q + side view each, EEVEE,
   to /tmp/eval_anim_r2_f<frame>_<view>.png
"""
import bpy
import os
import math
...
for f in frames:
    ...
    for vname, loc in VIEWS.items():
        ...
        scene.render.filepath = f"/tmp/eval_anim_r2_f{f:03d}_{vname}.png"
        bpy.ops.render.render(write_still=True)
        print("[render]", scene.render.filepath)

print(f"=== EVAL_ANIM_R2 DONE ({TAG}) ===")
