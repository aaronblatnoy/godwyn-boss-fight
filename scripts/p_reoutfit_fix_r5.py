"""
RE-OUTFIT Phase 4 fixer r5 VERIFY — beauty + face + detail + deform renders.
...
GPU (OptiX/CUDA) device is enabled (CPU fallback = hard failure), then renders:
  ...
  1. renders/wip/ro_f5_full.png        — full-body beauty (Cam_Full/Cam_Sheet)
  2. renders/wip/ro_f5_face.png        — face close-up (Cam_Face)
  3. renders/wip/ro_f5_back.png        — back 3/4 (cape + rear plate check)
  4. renders/wip/ro_f5_feet.png        — NEW: lower-body detail (sabatons +
       greaves + faulds close inspection)
  5. renders/wip/ro_f5_torso.png       — NEW: cuirass/pauldron/gorget detail
  6. renders/wip/ro_f5_deform.png      — DEFORM TEST: arm raise + forearm curl
       + spine twist + finger grip + knee bend + Expr blendshape 0.7
  7. renders/wip/ro_f5_deform_face.png — driven expression close-up
...
Run:
  blender --background models/godwyn_phase1.blend \
      --python scripts/p_reoutfit_fix_r5.py
"""
import math
import os
import sys
...
def _pose_rotate(arm, bone, axis, deg):
    ...
    if pb is None:
        print(f"[ro f5] WARN: bone {bone} missing")
        return
...
def anim_audit():
    ...
    print(f"[ro f5] ARMATURES ({len(arms)}): {[a.name for a in arms]}")
    ...
    print(f"[ro f5] CHARACTER MESHES ({len(meshes)}):")
    for m in sorted(meshes, key=lambda o: o.name):
        ...
        print(f"[ro f5]  {m.name}: verts={nverts} skinned={skinned} "
    ...
    print(f"[ro f5] SHAPE KEYS ({len(keys)}): {keys}")
    for mat in bpy.data.materials:
        if mat.use_nodes and mat.node_tree:
            dm = getattr(mat.cycles, "displacement_method", None)
            if dm in ("DISPLACEMENT", "BOTH"):
                print(f"[ro f5]  NOTE: material {mat.name} geometry "
...
def main():
    ...
    print(f"[ro f5] GPU OK: {dev_type} — " + ", ".join(d.name for d in gpus))
    ...
    print(f"[ro f5] grip bones curled: {len(grip)}")
    ...
    for i, v in enumerate(kb.data):
        ...
        if d > 1e-6:
            moved += 1
            mx = max(mx, d)
    print(f"[ro f5] blendshape '{kb.name}': {moved} verts moved, "
          f"max delta {mx*1000:.2f} mm")
    assert moved > 0, "FATAL: blendshape has zero vertex deltas"

    shoot(_pick(_DEFORM_CANDIDATES), "ro_f5_deform.png", 1280, 1600)
    shoot("Cam_Face", "ro_f5_deform_face.png", 1280, 1280)
    print("[ro f5] critic renders complete (scene NOT saved).")
