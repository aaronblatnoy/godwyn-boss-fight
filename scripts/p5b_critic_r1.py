"""
Phase 5 critic (DETAIL+ANIM pass, round 1) — beauty + deform-test renders.
...
GPU (OptiX/CUDA) device is enabled (CPU fallback = hard failure), then renders:
  ...
  1. renders/wip/p5b_r1_full.png   — full-body beauty (Cam_Full fallback chain)
  2. renders/wip/p5b_r1_face.png   — face close-up (Cam_Face)
  3. renders/wip/p5b_r1_deform.png — DEFORM TEST: bent arm + elbow crook +
...
Run:
  blender --background models/godwyn_phase1.blend \
      --python scripts/p5b_critic_r1.py
"""
import math
import os
import sys
...
def anim_audit():
    ...
    print(f"[p5b] ARMATURES ({len(arms)}): {[a.name for a in arms]}")
    ...
    for m in meshes:
        ...
        print(f"[p5b]  mesh {m.name}: skinned={skinned} "
    ...
    print(f"[p5b] SHAPE KEYS ({len(keys)}): {keys}")
    ...
    for mat in bpy.data.materials:
        if mat.use_nodes and mat.node_tree:
            ...
            if dm == "DISPLACEMENT" or dm == "BOTH":
                print(f"[p5b]  NOTE: material {mat.name} uses geometry "
...
def main():
    ...
    print(f"[p5b] GPU OK: {dev_type} — " + ", ".join(d.name for d in gpus))
    ...
    def shoot(cam, out_name, rx, ry):
        ...
        print(f"[p5b] rendering {cam} -> {out}")
        bpy.ops.render.render(write_still=True)
        assert os.path.isfile(out) and os.path.getsize(out) > 1024, (
            f"FATAL: render missing: {out}")
        print(f"[p5b] wrote {out} ({os.path.getsize(out)//1024} KB)")

    full_cam = _pick(_FULL_CANDIDATES)
    shoot(full_cam, "p5b_r1_full.png", 1280, 1600)
    shoot("Cam_Face", "p5b_r1_face.png", 1280, 1280)
    ...
    if kb is None:
        ...
        kb = others[0]
    kb.value = 0.7
    print(f"[p5b] deform pose applied; blendshape '{kb.name}'=0.7")
    bpy.context.view_layer.update()

    shoot(_pick(_DEFORM_CANDIDATES), "p5b_r1_deform.png", 1280, 1600)
    print("[p5b] critic renders complete (scene NOT saved).")
