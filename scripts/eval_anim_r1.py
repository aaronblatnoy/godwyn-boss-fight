"""
Movement-eval round 1 — animation frames from godwyn_p2_robe.blend.
...
4. Renders 4 frames across the motion, 3q + side view each, EEVEE,
   to /tmp/eval_anim_r1_f<frame>_<view>.png
"""
import bpy
import os
import math
...
# ── 2. procedural spring sway on phys chains ────────────────────────────────
CHAIN_AMP = {"phys_robe": 7.0, "phys_cape": 9.0, "phys_hair": 8.0}
def apply_sway(frame, span):
    t = (frame - span[0]) / max(1.0, span[1] - span[0])
    for name in phys_bones:
        pb = arm.pose.bones[name]
        base = name.rsplit("_", 1)[0]           # chain name
        link = int(name.rsplit("_", 1)[1])      # link index
        fam = next((k for k in CHAIN_AMP if name.startswith(k)), None)
        amp = CHAIN_AMP.get(fam, 6.0)
        # trailing wave: deeper links lag in phase, slightly larger amplitude
        ang = amp * (0.6 + 0.15 * link) * sin(2 * pi * (t * 1.5) - link * 0.7)
        side = 0.35 * amp * sin(2 * pi * t * 1.5 + hash(base) % 7 - link * 0.5)
        pb.rotation_mode = "XYZ"
        pb.rotation_euler = (radians(ang), 0.0, radians(side * 0.3))
    bpy.context.view_layer.update()
...
for f in frames:
    ...
    for vname, loc in VIEWS.items():
        cam.location = loc
        look_at(cam, TARGET)
        scene.render.filepath = f"/tmp/eval_anim_r1_f{f:03d}_{vname}.png"
        bpy.ops.render.render(write_still=True)
        print("[render]", scene.render.filepath)

print(f"=== EVAL_ANIM_R1 DONE ({TAG}) ===")
