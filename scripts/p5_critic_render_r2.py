"""
Phase 5 critic round 2 — beauty renders for art-direction review.
...
at review-quality samples with denoising to renders/wip/p5_r2_*.png.

Run:
  blender --background models/godwyn_phase1.blend \
      --python scripts/p5_critic_render_r2.py
"""
import os
import sys
...
def main():
    ...
    print(f"[p5r2] full-body camera: {full_cam}")

    shots = [
        (full_cam,   "p5_r2_full.png", (1280, 1600)),
        ("Cam_Face", "p5_r2_face.png", (1280, 1280)),
    ]
    ...
    for cam_name, out_name, (rx, ry) in shots:
        ...
        print(f"[p5r2] rendering {cam_name} -> {out} "
              f"({rx}x{ry} @ {SAMPLES}spp, denoise on)")
        bpy.ops.render.render(write_still=True)
        assert os.path.isfile(out), f"FATAL: render missing: {out}"
        print(f"[p5r2] wrote {out} ({os.path.getsize(out)} bytes)")

    print("[p5r2] critic renders complete.")
