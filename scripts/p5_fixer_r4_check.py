"""
Phase 5 fixer round 4 — verification renders (full / face / right hand).
...
(OptiX/CUDA) device (CPU fallback = hard failure), renders:
  p5_fix4_full.png  — Cam_Full (r4: 3/4-nudged so the grip reads), figure
                      vs the now-distant void crack
  p5_fix4_face.png  — Cam_Face: nose/mouth/brow relief, symmetric lidded
                      eyes with round catchlights, parted hairline + braid
  p5_fix4_hand.png  — temp camera on the right-hand sword grip
...
def main():
    ...
    print(f"[p5fix4] GPU OK: {dev_type}")
    ...
    shots = [
        ("Cam_Full", "p5_fix4_full.png", (1280, 1600)),
        ("Cam_Face", "p5_fix4_face.png", (1280, 1280)),
        ("_HandCam", "p5_fix4_hand.png", (1024, 1024)),
    ]
    for cam_name, out_name, (rx, ry) in shots:
        ...
        print(f"[p5fix4] rendering {cam_name} -> {out}")
        bpy.ops.render.render(write_still=True)
        assert os.path.isfile(out), f"FATAL: render missing: {out}"
        print(f"[p5fix4] wrote {out} ({os.path.getsize(out)} bytes)")
    print("[p5fix4] verification renders complete.")
