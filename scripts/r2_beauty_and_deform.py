# r2_beauty_and_deform.py — Round 2 evaluator renders (after rig hardening).
#   (a) CYCLES+OptiX beauty shot in a reflective dark-fantasy environment
#       (round-1 fix #3: EEVEE's screen-space reflections undersold the gold;
#        Cycles gives real raytraced reflections off the wet-stone floor).
#   (b) POSED DEFORM TEST — same sword-swing pose as round 1 so the
#       candy-wrapper / robe-clip fixes are directly comparable.
...
# ---------------- CYCLES + OptiX (GPU-real; CPU fallback is a failure)
...
print(f"[r2] CYCLES OptiX GPUs: {n_gpu}")
...
print("[r2] H", round(Hgt, 3), "center", tuple(round(v, 3) for v in center))

# ---------------- reflective dark-fantasy environment: gradient world + floor
w = bpy.data.worlds.new("R2World")
scn.world = w
w.use_nodes = True
wnt = w.node_tree
wnt.nodes.clear()
...
# reflective wet-stone floor (raytraced in Cycles)
floormat = bpy.data.materials.new("R2Floor")
...
# ================= (a) BEAUTY SHOT (rest pose, raytraced reflections)
shoot("RCamBeauty", 50, center.z, Hgt, 22.0, f"{OUT}/r2_beauty.png")

# ================= (b) POSED DEFORM TEST (same pose as round 1)
...
def rot(bone, x=0, y=0, z=0):
    b = pb.get(bone)
    if not b:
        print("[r2] MISSING bone", bone); return
    b.rotation_mode = "XYZ"
...
shoot("RCamPose", 50, center.z, Hgt, 28.0, f"{OUT}/r2_pose_deform.png")
shoot("RCamJoint", 80, bb_min.z + 0.72 * Hgt, 0.5 * Hgt, 40.0, f"{OUT}/r2_pose_joint.png")
print("[r2] DONE")
