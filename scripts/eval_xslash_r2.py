"""
eval_xslash_r2.py — Round-2 animation evaluation render of the X-slash.
...
frames with EEVEE to /tmp/xslash_r2_frames/, and renders labeled key stills
to /tmp/xslash_r2_strip/.

Run:
  blender --background --python ~/godwyn-boss-fight/scripts/eval_xslash_r2.py 2>&1
Then (server):
  ffmpeg -y -framerate 30 -i /tmp/xslash_r2_frames/f%04d.png -c:v libx264 \
    -pix_fmt yuv420p /tmp/xslash_r2.mp4
  ffmpeg -y -pattern_type glob -i '/tmp/xslash_r2_strip/*.png' \
    -filter_complex tile=4x2 /tmp/xslash_r2_strip.png
"""
import bpy, os, math
from mathutils import Euler, Vector

BLEND = os.path.expanduser("~/godwyn-boss-fight/models/godwyn_xslash.blend")
FRAMES_DIR = "/tmp/xslash_r2_frames"
STRIP_DIR = "/tmp/xslash_r2_strip"
...
print(f"[r2] opened {BLEND}  frames {sc.frame_start}-{sc.frame_end}  fps {sc.render.fps}")

# ── Verify animation content (body keys + cape bake) ────────────────────────
act = arm.animation_data.action if arm.animation_data else None
assert act is not None, "[r2] FATAL: armature has no action"
fcs = []
...
print(f"[r2] action '{act.name}': {len(fcs)} fcurves, "
      f"{len(body_keyed)} body bones keyed, {len(phys_keyed)} phys bones keyed")
assert len(body_keyed) >= 10, "[r2] FATAL: body animation missing"
assert len(phys_keyed) >= 30, "[r2] FATAL: cape bake missing (phys_* not keyed)"
...
print(f"[r2] animation frames -> {FRAMES_DIR}")
...
for i, (f, note) in enumerate(STRIP):
    ...
    print(f"[r2] still {sc.render.filepath}")
print("[r2] DONE")
