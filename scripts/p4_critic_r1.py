"""
REFINEMENT Phase 4 critic (round 1) — art director + anim QC renders.
...
GPU (OptiX/CUDA) device is enabled (CPU fallback = hard failure), then renders:
  ...
  1. renders/wip/c_r1_full.png        — full-body beauty (Cam_Full/Cam_Sheet)
  2. renders/wip/c_r1_face.png        — face close-up (Cam_Face)
  3. renders/wip/c_r1_back.png        — back 3/4 (must NOT read as a cape)
  4. renders/wip/c_r1_tabard.png      — NEW: front lower-body detail (tabard
       panel waist->floor + embroidery hem + greaves/sabatons seating)
  5. renders/wip/c_r1_torso.png       — cuirass/emblem/pauldron detail
  6. renders/wip/c_r1_deform.png      — DEFORM TEST: mild NATURAL action pose
       (guard-ish: arm raise + forearm curl + slight torso twist + finger grip
       + weight-shifted knee) + Expr_BrowSorrow 0.7
  7. renders/wip/c_r1_deform_face.png — driven expression close-up
...
Run:
  blender --background models/godwyn_phase1.blend \
      --python scripts/p4_critic_r1.py
"""
import math
import os
import sys
...
def _pose_rotate(arm, bone, axis, deg):
    ...
    if pb is None:
        print(f"[c r1] WARN: bone {bone} missing")
        return
...
def anim_audit():
    ...
    print(f"[c r1] ARMATURES ({len(arms)}): {[a.name for a in arms]}")
    ...
    print(f"[c r1] CHARACTER MESHES ({len(meshes)}):")
    cape_like = []
    for m in sorted(meshes, key=lambda o: o.name):
        ...
        print(f"[c r1]  {m.name}: verts={nverts} skinned={skinned} "
    ...
    print(f"[c r1] CAPE-NAMED MESHES (should be []): {cape_like}")
    ...
    print(f"[c r1] SHAPE KEYS ({len(keys)}): {keys}")
    ...
    if tab is not None:
        ...
        print(f"[c r1] TABARD: {tab.name} verts={len(tab.data.vertices)} "
              f"vgroups={groups}")
    else:
        print("[c r1] TABARD: WARNING — no mesh named *tabard*/*surcoat*")
    for mat in bpy.data.materials:
        if mat.use_nodes and mat.node_tree:
            dm = getattr(mat.cycles, "displacement_method", None)
            if dm in ("DISPLACEMENT", "BOTH"):
                print(f"[c r1]  NOTE: material {mat.name} geometry "
...
def main():
    ...
    print(f"[c r1] GPU OK: {dev_type} — " + ", ".join(d.name for d in gpus))
    ...
    def shoot(cam, out_name, rx, ry):
        ...
        print(f"[c r1] rendering {cam} -> {out}")
        bpy.ops.render.render(write_still=True)
        assert os.path.isfile(out) and os.path.getsize(out) > 1024, (
            f"FATAL: render missing: {out}")
        print(f"[c r1] wrote {out} ({os.path.getsize(out)//1024} KB)")

    full_cam = _pick(_FULL_CANDIDATES)
    shoot(full_cam, "c_r1_full.png", 1280, 1600)
    shoot("Cam_Face", "c_r1_face.png", 1280, 1280)
    back_cam = _pick(_BACK_CANDIDATES, allow_none=True)
    if back_cam:
        shoot(back_cam, "c_r1_back.png", 1280, 1600)

    # front lower-body: tabard panel + hem embroidery + greaves/sabatons
    _detail_cam("Cam_C1_Tabard", (0.9, -4.2, 1.05), (0.0, 0.0, 0.85), 55)
    shoot("Cam_C1_Tabard", "c_r1_tabard.png", 1400, 1500)
    _detail_cam("Cam_C1_Torso", (-2.2, -3.0, 2.35), (0.0, 0.0, 2.15), 60)
    shoot("Cam_C1_Torso", "c_r1_torso.png", 1400, 1400)
    ...
    print(f"[c r1] grip bones curled: {len(grip)}")
    ...
    for i, v in enumerate(kb.data):
        ...
        if d > 1e-6:
            moved += 1
            mx = max(mx, d)
    print(f"[c r1] blendshape '{kb.name}': {moved} verts moved, "
          f"max delta {mx*1000:.2f} mm")
    assert moved > 0, "FATAL: blendshape has zero vertex deltas"

    shoot(_pick(_DEFORM_CANDIDATES), "c_r1_deform.png", 1280, 1600)
    shoot("Cam_Face", "c_r1_deform_face.png", 1280, 1280)
    print("[c r1] critic renders complete (scene NOT saved).")
