"""
Phase 5 fixer round 3 — verification renders (full / face / right hand).
...
(OptiX/CUDA) device (CPU fallback = hard failure), renders:
  p5_fix3_full.png  — Cam_Full, whole figure vs the void crack
  p5_fix3_face.png  — Cam_Face, eyes/lids/hairline/expression
  p5_fix3_hand.png  — temp camera on the right-hand sword grip

Run:
  blender --background models/godwyn_phase1.blend \
      --python scripts/p5_fixer_r3_check.py
"""
import os
import sys
...
def main():
    ...
    print(f"[p5fix3] GPU OK: {dev_type}")
    ...
    shots = [
        ("Cam_Full", "p5_fix3_full.png", (1280, 1600)),
        ("Cam_Face", "p5_fix3_face.png", (1280, 1280)),
        ("_HandCam", "p5_fix3_hand.png", (1024, 1024)),
    ]
    for cam_name, out_name, (rx, ry) in shots:
        ...
        print(f"[p5fix3] rendering {cam_name} -> {out}")
        bpy.ops.render.render(write_still=True)
        assert os.path.isfile(out), f"FATAL: render missing: {out}"
        print(f"[p5fix3] wrote {out} ({os.path.getsize(out)} bytes)")
    print("[p5fix3] verification renders complete.")
