"""
Movement-eval round 3 — animation frames from godwyn_p2_robe.blend.
...
4. Renders 4 frames across the motion, 3q + side view each, EEVEE,
   to /tmp/eval_anim_r3_f<frame>_<view>.png
"""
import bpy
import os
import math
...
# FIXER R2: pure sine sway left the cape a rigid plane sticking straight out
# whenever the torso pitched (the f097 leaping slam). Approximate what Godot
# spring bones will do: per link, rotate the bone a fraction K toward world
# -Z so extreme poses drape downward instead of staying board-rigid, then add
# a small trailing sine wave on top.
CHAIN_AMP = {"phys_robe": 5.0, "phys_cape": 7.0, "phys_hair": 6.0}
DRAPE_K = {"phys_robe": 0.30, "phys_cape": 0.45, "phys_hair": 0.35}
...
def apply_sway(frame, span):
    ...
    for name in sorted(phys_bones):
        ...
        amp = CHAIN_AMP[fam]
        k = DRAPE_K[fam] * (0.6 + 0.10 * link)     # deeper links hang more
        d = (pb.matrix.to_3x3() @ Vector((0, 1, 0))).normalized()
        target = (d * (1.0 - k) + down * k)
        q = Quaternion()
        if target.length > 1e-5 and d.cross(target.normalized()).length > 1e-5:
            tgt = target.normalized()
            q = Quaternion(d.cross(tgt).normalized(), d.angle(tgt))
        ...
        pb.matrix = T @ (qs @ q).to_matrix().to_4x4() @ T.inverted() @ pb.matrix
        bpy.context.view_layer.update()
...
for f in frames:
    ...
    for vname, loc in VIEWS.items():
        ...
        scene.render.filepath = f"/tmp/eval_anim_r3_f{f:03d}_{vname}.png"
        bpy.ops.render.render(write_still=True)
        print("[render]", scene.render.filepath)

print(f"=== EVAL_ANIM_R3 DONE ({TAG}) ===")
