"""
RE-OUTFIT Phase 4 critic (round 3) — beauty + face + detail + deform renders.
...
GPU (OptiX/CUDA) device is enabled (CPU fallback = hard failure), then renders:
  ...
  1. renders/wip/ro_c3_full.png        — full-body beauty (Cam_Full/Cam_Sheet)
  2. renders/wip/ro_c3_face.png        — face close-up (Cam_Face)
  3. renders/wip/ro_c3_back.png        — back 3/4 (cape + rear plate check)
  4. renders/wip/ro_c3_feet.png        — NEW: lower-body detail (sabatons +
       greaves + faulds close inspection)
  5. renders/wip/ro_c3_torso.png       — NEW: cuirass/pauldron/gorget detail
  6. renders/wip/ro_c3_deform.png      — DEFORM TEST: arm raise + forearm curl
       + spine twist + finger grip + knee bend + Expr blendshape 0.7
  7. renders/wip/ro_c3_deform_face.png — driven expression close-up
...
Run:
  blender --background models/godwyn_phase1.blend \
      --python scripts/p_reoutfit_critic_r3.py
"""
import math
import os
import sys
...
def _pose_rotate(arm, bone, axis, deg):
    ...
    if pb is None:
        print(f"[ro c3] WARN: bone {bone} missing")
        return
...
def anim_audit():
    ...
    print(f"[ro c3] ARMATURES ({len(arms)}): {[a.name for a in arms]}")
    ...
    print(f"[ro c3] CHARACTER MESHES ({len(meshes)}):")
    for m in sorted(meshes, key=lambda o: o.name):
        ...
        print(f"[ro c3]  {m.name}: verts={nverts} skinned={skinned} "
    ...
    print(f"[ro c3] SHAPE KEYS ({len(keys)}): {keys}")
    for mat in bpy.data.materials:
        if mat.use_nodes and mat.node_tree:
            dm = getattr(mat.cycles, "displacement_method", None)
            if dm in ("DISPLACEMENT", "BOTH"):
                print(f"[ro c3]  NOTE: material {mat.name} geometry "
...
def main():
    ...
    print(f"[ro c3] GPU OK: {dev_type} — " + ", ".join(d.name for d in gpus))
    ...
    def shoot(cam, out_name, rx, ry):
        ...
        print(f"[ro c3] rendering {cam} -> {out}")
        bpy.ops.render.render(write_still=True)
        assert os.path.isfile(out) and os.path.getsize(out) > 1024, (
            f"FATAL: render missing: {out}")
        print(f"[ro c3] wrote {out} ({os.path.getsize(out)//1024} KB)")

    full_cam = _pick(_FULL_CANDIDATES)
    shoot(full_cam, "ro_c3_full.png", 1280, 1600)
    shoot("Cam_Face", "ro_c3_face.png", 1280, 1280)
    back_cam = _pick(_BACK_CANDIDATES, allow_none=True)
    if back_cam:
        shoot(back_cam, "ro_c3_back.png", 1280, 1600)

    # --- NEW round-3 inspection cameras (feet + torso detail) ---------------
    _detail_cam("Cam_RO3_Feet", (-2.6, -3.4, 0.75), (0.05, 0.0, 0.55), 60)
    shoot("Cam_RO3_Feet", "ro_c3_feet.png", 1400, 1200)
    _detail_cam("Cam_RO3_Torso", (-2.2, -3.0, 2.35), (0.0, 0.0, 2.15), 60)
    shoot("Cam_RO3_Torso", "ro_c3_torso.png", 1400, 1400)
    ...
    print(f"[ro c3] grip bones curled: {len(grip)}")
    ...
    for i, v in enumerate(kb.data):
        ...
        if d > 1e-6:
            moved += 1
            mx = max(mx, d)
    print(f"[ro c3] blendshape '{kb.name}': {moved} verts moved, "
          f"max delta {mx*1000:.2f} mm")
    assert moved > 0, "FATAL: blendshape has zero vertex deltas"

    shoot(_pick(_DEFORM_CANDIDATES), "ro_c3_deform.png", 1280, 1600)
    shoot("Cam_Face", "ro_c3_deform_face.png", 1280, 1280)
    print("[ro c3] critic renders complete (scene NOT saved).")
